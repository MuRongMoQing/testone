...
- Linux（当前受阻）：目标依赖仍为 `libcpp-httplib-dev`、`libmysqlclient-dev`、`libsodium-dev`、`libssl-dev`、`nlohmann-json3-dev` 和 `pkg-config`，但 Ubuntu 24.04 的 `libcpp-httplib-dev` 不提供当前 CMake 所要求的 config package；修复并通过 CI 前不得把该路径描述为可用
...
Linux 构建当前在 CMake 发现 `httplib` 时失败，尚无已验证通过的 Linux 命令。修复约束和真实 CI 结果见 [开发指南](docs/DEVELOPMENT.md) 与 [实施计划](docs/IMPLEMENTATION_PLAN.md)；不得以依赖安装成功代替配置、构建或测试通过。
