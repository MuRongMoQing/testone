...
目标系统采用 C++17 模块化单体：一个 `warehouse_server` 进程、一个 MySQL 8 数据库和清晰的进程内模块边界。当前 `src/main.cpp` 仍同时承担套接字、HTTP、JSON、鉴权、SQL、迁移和业务规则；下述内容是尚待分阶段实现的目标，完成重构后 `main.cpp` 只读取配置、组装适配器、注册路由并控制启动与停止。
...
首版通过 vcpkg 锁定 `cpp-httplib` 和 `nlohmann-json`，替换现有套接字报文解析、正则 JSON 提取和每连接分离线程。HTTP 适配器使用有界线程池，并通过部署配置设置请求头、普通 JSON、multipart 总体、单附件、读取、写入和空闲超时限制；附件仍执行既定 20 MiB 单文件限制及安全扫描。

API 只接受声明的 JSON 字段类型，拒绝语法错误、类型错误、超出精度或限制的内容，并使用统一错误对象映射应用错误。CORS 使用显式来源允许列表，不再返回通配符。路由、认证头、错误状态、JSON 字段和 multipart 行为由协议测试覆盖，使协议库替换不改变既有外部契约。
